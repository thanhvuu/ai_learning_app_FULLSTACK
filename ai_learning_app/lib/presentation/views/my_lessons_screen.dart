...
class _MyLessonsScreenState extends State<MyLessonsScreen> {
  ...
  Future<void> fetchMyLessons() async {
    ...
    final String url =
        "${ApiConfig.lessons}/my-lessons?username=$encodedUsername";
  ...
  List<Color> _getGradientForType(String type) {
    if (type == 'drag_drop') {
      return const [Color(0xFF2E86C1), Color(0xFF85C1E9)]; // Xanh biển
    }
    if (type == 'multiple_choice') {
      return const [Color(0xFF935116), Color(0xFFE59866)]; // Cam gỗ
    }
    if (type == 'fill_blank') {
      return const [Color(0xFF17202A), Color(0xFF5D6D7E)]; // Xám công nghệ
    }
            ...
            child: Stack(
              ...
              children: [
                CircularProgressIndicator(
                  value: minutesLearned / dailyGoal,
                  strokeWidth: 10,
                ...
                Center(
                  child: Text(
                    "${((minutesLearned / dailyGoal) * 100).toInt()}%",
                    style: TextStyle(
                      fontWeight: FontWeight.bold,
                      fontSize: 20,
                      color: textColor,
